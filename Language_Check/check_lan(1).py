    ...
    try:
        ...
        for r in list(range(rows_num))[x:]:  # 取第4行到最后一行
            for c in list(range(clos_num))[y:]:  # 取第9列到最后一行
                b = sheet.cell(row=r, column=c) #单元格
                a = b.value  #每个单元格的值
                ...
                # language, confidence = identifier.classify("欢迎来到指尖填色")
                # print(f"Language: {language}, Confidence: {confidence}")

                # identifier = LanguageIdentifier.from_modelstring(model, norm_probs=True)
                # language, confidence = identifier.classify(a)
                # # print(f"Language: {language}, Confidence: {confidence}")
                # lan = sheet.cell(row=2, column=c).value
                # if language != lan and confidence >= 0.99:
                ...
                ###检查非中文语系是否有中文标点符号
                language = sheet.cell(row=2, column=c).value
                if language != "zh-CN" and language != "zh-TW" and language != "ja" and language != "zh" :
                    if a.find("！") != -1:
                ...
                ###检查特殊符号是否遗漏
                d = sheet.cell(row=r, column=c).value
                if d != None:
                    if d.find("%") != -1:
                        if a.find("%") == -1:
